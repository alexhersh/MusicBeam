...
Toggle aHueToggle, mirrorToggle, gradToggle;
...
Derby_Effect(MusicBeam ctrl, int y)
{
        ...
        gradToggle = cp5.addToggle("grad"+getName()).setPosition(0, 255).setSize(395, 45).setGroup(controlGroup);
...
void draw()
{
        ...
        if (gradToggle.getState())
        {
          ...
          stg.fill(hueSlider.getValue()+120, 100, 100);
        }
        for (int i=1; i<=points; i++)
        {
