...
Toggle aHueToggle, stereoToggle, bwToggle, gradToggle;
...
void draw()
{
        ...
        if (!gradToggle.getState())
        {
                pair[0] = color(hueSlider.getValue()%360, bwToggle.getState() ? 0 : 100, 100);
                pair[1] = color((hueSlider.getValue()+120)%360, bwToggle.getState() ? 0 : 100, 100);
        }
